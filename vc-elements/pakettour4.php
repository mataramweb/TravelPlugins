...
*/
// Element Class 
class model4 extends WPBakeryShortCode {
     
    // Element Init
    function __construct() {
        add_action( 'init', array( $this, 'mapping_v4' ) );
        add_shortcode( 'package_v4', array( $this, 'html_v4' ) );
    }
     
    // Element Mapping
    public function mapping_v4() {
...
foreach( $categories as $category ){
    $categories_array[] = $category->name;
        ...
        vc_map( 
            array(
                'name' => __('Paket Tour V-4', 'text-domain'),
                'base' => 'package_v4',
...
<?php
  if (have_posts()) :
          while(have_posts()) : the_post();
			...
			$meta_data = get_post_meta(get_the_ID(), 'slide_options');
...
$container = 	'<div  class="row">%1$s</div>';
 ...
 <a href="'.$href.'">
							<div class="activities_img" style="background-image:url('.$featured_img_url.')"></div>
